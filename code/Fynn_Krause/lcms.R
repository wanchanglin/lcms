...
dbase <- makelibrary(sel.class, fixed=F, fixed_FA, lookup_lipid_class, lookup_FA, lookup_element)
...
xset2 <- retcor(xset, family = "symmetric", plottype = "mdevden", smooth = "loess", span = 0.2, missing = 5, extra = 1) # corrects retention times
#xset2 <- retcor(xset, method="obiwarp", profStep=0.1, plottype="deviation")  # different method for retention time correction - more robust but slower
...
# deisotoping - working with raw peak area - can also change "out" to use for peak height or the fitered results (value = into/intf/maxo/maxf)
